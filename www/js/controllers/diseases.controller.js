...
.controller('DiseasesCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.diseases = {};
  $scope.diseases.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_diseases/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_diseases',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "diseases") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertVaccination = function(diseases){
    diseases.date = new Date(diseases.date);
    diseases.type = "diseases";
    db.post(diseases).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('diseases-modal.html', {
    scope: $scope,
