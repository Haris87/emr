...
.controller('SurgeriesCtrl', function($scope, $stateParams, $location, $ionicModal, $timeout, $ionicPopup) {
  ...
  $scope.surgeries = {};
  $scope.surgeries.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_surgeries',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "surgeries") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertSurgeries = function(surgeries){
	var input = document.getElementById('file');
	var file = input.files[0];

	if(typeof file != 'undefined'){
	  surgeries._attachments = {
	    file: {
		  data: file,
    ...
    surgeries.date = new Date(surgeries.date);
    surgeries.type = "surgeries";
    db.post(surgeries).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('surgeries-modal.html', {
    scope: $scope,
