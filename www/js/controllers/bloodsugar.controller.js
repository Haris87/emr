...
.controller('BloodSugarCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.bloodsugar = {};
  $scope.bloodsugar.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_bloodsugar/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_bloodsugar',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "bloodsugar") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertBloodSugar = function(bloodsugar){
    bloodsugar.date = new Date(bloodsugar.date);
    bloodsugar.type = "bloodsugar";
    db.post(bloodsugar).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('blood-sugar-modal.html', {
    scope: $scope,
