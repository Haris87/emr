...
.controller('SymptomReportsCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.symptoms = {};
  $scope.symptoms.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_symptoms/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_symptoms',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "symptoms") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertSymptom = function(symptoms){
    symptoms.date = new Date(symptoms.date);
    symptoms.type = "symptoms";
    db.post(symptoms).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('symptoms-modal.html', {
    scope: $scope,
