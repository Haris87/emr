...
.controller('DisabilitiesCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.disabilities = {};
  $scope.disabilities.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_disabilities/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_disabilities',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "disabilities") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertDisability = function(disabilities){
    disabilities.date = new Date(disabilities.date);
    disabilities.type = "disabilities";
    db.post(disabilities).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('disabilities-modal.html', {
    scope: $scope,
