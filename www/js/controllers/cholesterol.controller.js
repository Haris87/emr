...
.controller('CholesterolCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.cholesterol = {};
  $scope.cholesterol.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_cholesterol/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_cholesterol',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "cholesterol") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertCholesterol = function(cholesterol){
    cholesterol.date = new Date(cholesterol.date);
    cholesterol.type = "cholesterol";
    db.post(cholesterol).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('cholesterol-modal.html', {
    scope: $scope,
