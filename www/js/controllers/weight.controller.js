...
.controller('WeightCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.weight = {};
  $scope.weight.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_weight/object').then(function (res) {
      console.log(res);
  ...
  $scope.insertWeight = function(weight){
    weight.date = new Date(weight.date);
    weight.type = "weight";
    db.post(weight).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('weight-modal.html', {
    scope: $scope,
