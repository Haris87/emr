...
.controller('HospitalisationsCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  var db = new PouchDB('emr', {auto_compaction: true});
  var id = "hospitalisations";
  ...
  $scope.hospitalisations.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_hospitalisations/object').then(function (res) {
      console.log(res);
      for(var i=0; i<res.rows.length; i++){
        var row = res.rows[i].value.doc;
        row.id = i;
        row.color = $scope.getColor(row);
		var discharge_file = null;

		if(typeof row._attachments != 'undefined'){
			db.getAttachment(row._id, 'discharge_file')
			.then(function (blob) {
			  console.log(blob);
		...
		console.log(row);
		row.discharge_file = discharge_file;
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_hospitalisations',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "hospitalisations") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertHospitalisations = function(hospitalisations){
	var input = document.getElementById('discharge');
	var discharge = input.files[0];

	if(typeof discharge != 'undefined'){
	  hospitalisations._attachments = {
	    discharge_file: {
		  data: discharge,
		  content_type: discharge.type
	    }
	  };
	}

    hospitalisations.date = new Date(hospitalisations.date);
    hospitalisations.type = "hospitalisations";
    db.post(hospitalisations).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('hospitalisations-modal.html', {
    scope: $scope,
