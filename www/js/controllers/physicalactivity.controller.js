...
.controller('PhysicalActivityCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  var db = new PouchDB('emr', {auto_compaction: true});
  var id = "physicalactivity";
  ...
  $scope.physicalactivity.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_physicalactivity/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_physicalactivity',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "physicalactivity") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertPhysicalActivity = function(physicalactivity){
    physicalactivity.date = new Date(physicalactivity.date);
    physicalactivity.type = "physicalactivity";
    db.post(physicalactivity).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('physical-activity-modal.html', {
    scope: $scope,
