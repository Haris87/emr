...
.controller('AllergiesCtrl', function($scope, $stateParams, $location, $ionicModal, $ionicPopup) {
  ...
  $scope.allergies = {};
  $scope.allergies.date = new Date();//.toJSON().slice(0,10);
  ...
  $scope.getMeasurements = function(){
    ...
    db.query('index_allergies/object').then(function (res) {
      console.log(res);
  ...
  $scope.createIndex = function(){
    ...
    var ddoc = {
      _id: '_design/index_allergies',
      views: {
        by_id: {
          map: function (doc) {
            if(doc.type == "allergies") {
              emit(doc.id);
            }
          }.toString()
        },
  ...
  $scope.insertAllergies = function(allergies){
    allergies.date = new Date(allergies.date);
    allergies.type = "allergies";
    db.post(allergies).then(function(response) {
  ...
  $ionicModal.fromTemplateUrl('allergies-modal.html', {
    scope: $scope,
